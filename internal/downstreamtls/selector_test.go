...
func TestCompileRejectsInvalidPolicies(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "wildcard SAN trailing dot", policy: &model.DownstreamTLSPolicy{DefaultCertificateRef: "default", SNIBindings: []model.SNIBinding{{CertificateRef: "wild-dot", Hosts: []string{"*.example.com"}}}}, certificates: []*tlsmaterial.Certificate{defaultCertificate, valid("wild-dot", 4, "*.example.com.")}, wantCode: CodeCertificateHostnameMismatch},
